...
public interface IBuffetOrderService extends BaseRepository<BuffetOrder, Integer>, JpaSpecificationExecutor<BuffetOrder> {
    ...
    @Query("FROM BuffetOrder b WHERE b.surplusBond<=0 and (b.status='2' or b.status='3') and b.isSelf='1'")
    List<BuffetOrder> findByNoBond();

    @Query("FROM BuffetOrder b WHERE b.status='0' AND b.isSelf='1'")
    List<BuffetOrder> findByNoPay();
